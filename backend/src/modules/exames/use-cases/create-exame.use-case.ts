...
export class CreateExameUseCase {
  ...
  async execute(createExameDto: {
    ...
    // Gerar chave de idempotência se não for fornecida
    let idempotencyKey = createExameDto.idempotencyKey;
    if (!idempotencyKey) {
      const base = createExameDto.nome_exame || 'exame';
      const timestamp = Date.now();
      idempotencyKey = `${base}-${timestamp}`.replace(/\s+/g, '-').toLowerCase();
    }
