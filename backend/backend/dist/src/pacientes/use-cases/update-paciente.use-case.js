"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdatePacienteUseCase = void 0;
const typeorm_1 = require("typeorm");
const paciente_entity_1 = require("../entities/paciente.entity");
const typeorm_2 = require("@nestjs/typeorm");
const common_1 = require("@nestjs/common");
let UpdatePacienteUseCase = class UpdatePacienteUseCase {
    pacienteRepo;
    constructor(pacienteRepo) {
        this.pacienteRepo = pacienteRepo;
    }
    async update(id, input) {
        const paciente = await this.pacienteRepo.findOneOrFail({ where: { id } });
        const mudancas = {};
        let houveAlteracao = false;
        const camposAtualizaveis = [
            'nome',
            'email',
            'data_nascimento',
            'telefone',
            'endereco',
            'documento_cpf',
            'sexo',
            'status'
        ];
        for (const campo of camposAtualizaveis) {
            if (input[campo] !== undefined && input[campo] !== paciente[campo]) {
                mudancas[campo] = input[campo];
                houveAlteracao = true;
            }
        }
        if (houveAlteracao) {
            mudancas.data_atualizacao = new Date();
            Object.assign(paciente, mudancas);
            await this.pacienteRepo.save(paciente);
        }
        return paciente;
    }
};
exports.UpdatePacienteUseCase = UpdatePacienteUseCase;
exports.UpdatePacienteUseCase = UpdatePacienteUseCase = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_2.InjectRepository)(paciente_entity_1.Paciente)),
    __metadata("design:paramtypes", [typeorm_1.Repository])
], UpdatePacienteUseCase);
//# sourceMappingURL=update-paciente.use-case.js.map