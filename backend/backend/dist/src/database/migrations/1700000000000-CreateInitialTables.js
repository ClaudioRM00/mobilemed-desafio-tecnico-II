...
class CreateInitialTables1700000000000 {
    async up(queryRunner) {
        ...
        await queryRunner.createIndex('paciente', new typeorm_1.TableIndex({ name: 'IDX_PACIENTE_CPF_UNIQUE', columnNames: ['documento_cpf'], isUnique: true }));
        await queryRunner.createTable(new typeorm_1.Table({
            name: 'exame',
            columns: [
                { name: 'id', type: 'varchar', isPrimary: true, length: '36' },
        ...
        await queryRunner.createIndex('exame', new typeorm_1.TableIndex({ name: 'IDX_EXAME_IDEMPOTENCY_UNIQUE', columnNames: ['idempotencyKey'], isUnique: true }));
        await queryRunner.createIndex('exame', new typeorm_1.TableIndex({ name: 'IDX_EXAME_PACIENTE', columnNames: ['id_paciente'] }));
